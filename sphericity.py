...
for mass in dFrames:
    ...
    print("d3c")
    filteredFrame = filteredFrame.Define("trackMagnitudes", "")
    filteredFrame = filteredFrame.Define("sphericityTensor", "double s[3][3]={{0,0,0},{0,0,0},{0,0,0}}; double denominator=0; for (int i=0; i<nTracks; i++) {denominator += sqrt(Tracksi[i].Mag2); for (int j=0; j<3; j++) {for (int k=0; k<3; k++) {s[j][k]+=}}}; return s;")
    # perp2 is pt squared
